...
func solve() int {
	...
	for _, word := range WORDS {
		if checkTriangle(getScore(word)) {
			res++
		}
	}
	return res
...
func getScore(s string) int {
	res := 0
	for _, c := range s {
		res += getPlace(string(c))
	}
	return res
}

func getPlace(s string) int {
...
func checkTriangle(n int) bool {
	for i := 1; ; i++ {
		if 0.5*float32(i)*(float32(i)+1) == float32(n) {
			return true
		} else if 0.5*float32(i)*(float32(i)+1) > float32(n) {
			return false
		}
	}
}
